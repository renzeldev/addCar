...
export class LocationListResolverService implements Resolve<ListPageWrapper<LocationListItem>> {
  ...
  resolve(
    ...
    let page = '1';
    const settings = this.listSettings.get(`/franchisees/locations`);
    ...
    if (route.params['pageNum']) {
      ...
      this.listSettings.set(`/franchisees/locations`, { page: page });
    } else if (settings && settings.page) {
      this.router.navigate([`/franchisees/locations/page/${settings.page}`]);
      return;
    }
