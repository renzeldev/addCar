...
export class NavigationService {
  ...
  constructor(private router: Router, private config: NavigationConfig, private authService: AuthService, private resizeService: ResizeService) {
    this.validateConfig();
  ...
  public setContextVariable(name: string, value: string, saveState: boolean = false) {
    if (saveState)
      this.contextVariableStateful[name] = value;
    else
      this.contextVariableStateless[name] = value;
    this.publishMenus();
  }
  ...
  private discoverSideMenu(): MenuItem[] {
    if (this.currentRoute && this.currentRoute.menuCode) {
      ...
      if (item) {
        if (this.currentRoute.menuCallback)
          this.currentRoute.menuCallback(item);
        return this.getMenuForItem(item);
      }
    }
    const items = this.menuRouteItems.filter(a => this.router.isActive(a.route, a.activeOptions.exact));
    if (items.length > 0)
      return this.getMenuForItem(items[0]);
  ...
  private applyItemContextVariable(item: MenuItem, variableName: string, variableValue: string) {
    //if(item.title == "Create $EntityName$")
    if (!variableValue)
      variableValue = '';
    if (item.icon)
      item.icon = item.icon.replace(`$${variableName}$`, variableValue);
    if (item.route)
      item.route = item.route.replace(`$${variableName}$`, variableValue);
    if (item.title)
      item.title = item.title.replace(`$${variableName}$`, variableValue);
    if (item.breadcrumbTitle)
      item.breadcrumbTitle = item.breadcrumbTitle.replace(`$${variableName}$`, variableValue);
    if (item.hiddenFlagName === variableName)
      item.isHidden = variableValue === "true";
  }
  ...
  private loadScreenSizeConfig(width: number): boolean {
    const configs = this.config.screenSizeConfigs.filter(a => a.maxResolution >= width).sort((a, b) => a.maxResolution - b.maxResolution);

    if (!this.currentScreenSizeConfig || configs[0].maxResolution !== this.currentScreenSizeConfig.maxResolution) {
  ...
  private validateConfig() {
    ...
    this.config.screenSizeConfigs.forEach(a => {
      if (this.config.screenSizeConfigs.filter(b => b.maxResolution === a.maxResolution).length > 1)
